...
class SpotBugsAnnotationProviderTest extends Specification {
    ...
    def 'check'() {
        given:
        def sut = new SpotBugsAnnotationProvider('src/main/java', FileUtil.readString(Paths.get('src/test/resources/spotbugs-test.xml')))
        ...
        sut.convert() == [
            ...
            new Annotation(30, 'io.jenkins.plugins.codeInsights.usecase.ExecutableAnnotationProvidersBuilder.setSonarQube(String, String, String, String, String) は，java.lang.Exception を無視しているかもしれません。', 'src/main/java/io/jenkins/plugins/codeInsights/usecase/ExecutableAnnotationProvidersBuilder.kt', Severity.LOW, null),
            new Annotation(30, 'io.jenkins.plugins.codeInsights.usecase.ExecutableAnnotationProvidersBuilder.setSonarQube(String, String, String, String, String) は，java.lang.Exception を無視しているかもしれません。', 'src/main/java/io/jenkins/plugins/codeInsights/usecase/ExecutableAnnotationProvidersBuilder.kt', Severity.LOW, null),
            new Annotation(36, 'io.jenkins.plugins.codeInsights.usecase.ExecutableAnnotationProvidersBuilder.build() は，ExecutableAnnotationProvidersBuilder.executables を返すことによって内部表現を暴露するかもしれません。', 'src/main/java/io/jenkins/plugins/codeInsights/usecase/ExecutableAnnotationProvidersBuilder.kt', Severity.LOW, null),
            new Annotation(65, 'Collection から抽象クラス java.util.List への疑わしいキャストです。io.jenkins.plugins.codeInsights.usecase.GitRepo.detectChangedFiles(String, String)', 'src/main/java/io/jenkins/plugins/codeInsights/usecase/GitRepo.kt', Severity.LOW, null),
            new Annotation(69, 'Collection から抽象クラス java.util.List への疑わしいキャストです。io.jenkins.plugins.codeInsights.usecase.GitRepo.detectChangedFiles(String, String)', 'src/main/java/io/jenkins/plugins/codeInsights/usecase/GitRepo.kt', Severity.LOW, null),
        ]
    }
}
